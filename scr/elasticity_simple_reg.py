...
def find_by_keywords(df, any_tokens, required=False):
    """
    Trouve la 1re colonne dont le nom (lowercase) contient au moins
    UN des tokens fournis (ex: ['dest','to','city']).
    """
...
def build_state_city_year_from_merged(df_merged: pd.DataFrame) -> pd.DataFrame:
    ...
    # Colonnes critiques avant agrégation
...
# -----------------------
# Bloc 1 : Elasticité au niveau states -> city
# -----------------------
def run_states_elasticity(df_state: pd.DataFrame):
    _ensure_out_dir()

    print("\n" + "="*70)
    print("[STATES] Elasticity (states → cities → year)")
    print("="*70)

    df = df_state.copy()

    # Colonnes de base (passagers/prix)
    ...
    # Contrôles potentiels (facultatifs)
    ...
    df = df[df[col_pax] >= 5000]  # seuil flux
    ...
    if cluster_var is not None:
        print(f"\n[INFO] Using clustered standard errors (groups = {cluster_name})")
    else:
    ...
    # 4) Graphique log–log (SAVE TO outputs/)
...
# -----------------------
# Bloc 2 : Elasticité au niveau city1 -> city2
# -----------------------
def run_cities_elasticity(df_merged: pd.DataFrame):
    _ensure_out_dir()

    print("\n" + "="*70)
    print("[CITIES] Elasticity (city1 → city2 → year)")
    print("="*70)
    ...
    try:
        ...
        fe_mod = PanelOLS(Y, Xfe, entity_effects=True, time_effects=True)
        fe_res = fe_mod.fit(cov_type="clustered", cluster_entity=True)
        print("\n=== [CITIES] Two-way FE (cities): route & year — clustered by route ===")
        print(f"Variables in X: {fe_X_cols}")
        print(fe_res.summary)
